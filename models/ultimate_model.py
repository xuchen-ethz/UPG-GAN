...
class VAECycleGANModelAll(BaseModel):
    def name(self):
        return 'VAECycleGANModelAll'
    ...
    def backward_D_A(self):
        ...
        fake_B_cond = torch.cat([self.fake_B, c_img], dim=1)
        fake_B_cond = self.fake_B_pool.query(fake_B_cond)

        real_B_cond = torch.cat([self.real_B, c_img], dim=1)
        loss_D_A = self.backward_D_basic(self.netD_A, real_B_cond, fake_B_cond)
    ...
    def backward_EG(self):
        ...
        c = self.opt.cond_nc
        c_img = self.real_C.view(b, c, 1, 1).expand(b,c,h,w)
        fake_B_cond = torch.cat([fake_B, c_img], dim=1)
        pred_fake = self.netD_A(fake_B_cond)
    ...
    def get_current_errors(self):
        ret_errors = OrderedDict([('D_A', self.loss_D_A), ('G_A', self.loss_G_A), ('Cyc_A', self.loss_cycle_A),
                                 ('D_B', self.loss_D_B), ('G_B', self.loss_G_B), ('Cyc_B',  self.loss_cycle_B),
                                  ('KL', self.loss_kl), ('z_L1', self.loss_z_L1.data[0])])
