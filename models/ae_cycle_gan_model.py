...
class AECycleGANModel(BaseModel):
    def name(self):
        return 'AECycleGANModel'
    ...
    def initialize(self, opt):
        ...
        self.netE = networks.define_E(opt.output_nc, opt.nz, opt.nef,
                                      ...
                                      init_type=opt.init_type, gpu_ids=self.gpu_ids, vaeLike=False, pooling=opt.pooling)

        which_G_A  = 'resnet_6blocks_all' if opt.fineSize < 200 else 'resnet_9blocks_all'
        self.netG_A = networks.define_G(opt.input_nc, opt.output_nc,
                                        opt.ngf, which_G_A, opt.norm, opt.dropout, opt.init_type, self.gpu_ids, nz=self.opt.nz, upsample=opt.upsample)

        self.netG_B = networks.define_G(opt.output_nc, opt.input_nc,
                                        opt.ngf, opt.which_model_netG, opt.norm, False, opt.init_type, self.gpu_ids, upsample=opt.upsample)

        if self.opt.median:
            self.median_filter = nn.Sequential(*[networks.MedianPool2d(9,1,same=True)])
    ...
    def backward_EG(self):
        ...
        real_z = self.netE.forward(self.real_B)
        self.real_z = real_z

        fake_B = self.netG_A(self.real_A, real_z)
        fake_A = self.netG_B(self.real_B)
        if self.opt.median:
            fake_A_med = self.median_filter(fake_A)
        else:
            fake_A_med = fake_A
        rec_B = self.netG_A(fake_A_med, real_z)
        rec_A = self.netG_B(fake_B)

        # Classification Loss
        if lambda_class > 0 and self.opt.cond_nc > 0:
            loss_class = self.criterionClass(real_z[:,:self.opt.cond_nc], self.real_C) * lambda_class
        ...
        if self.opt.lambda_z > 0.0:
            self.fake_z = self.netE.forward(fake_B)
        ...
        # combined loss
        loss_G = loss_G_A + loss_G_B + loss_cycle_A + loss_cycle_B + loss_tv_B + loss_tv_A + loss_class
    ...
    def backward_G_alone(self):
        ...
        if self.opt.lambda_z > 0.0:
            self.loss_z_L1 = torch.mean(torch.abs(self.fake_z - self.real_z)) * self.opt.lambda_z
